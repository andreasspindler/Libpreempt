...
namespace base {
  ...
  /**
   * Define RLIM_INFINITY for RLIMIT_MEMLOCK effectively allowing the process to
   * lock all virtual RAM.
   */
  bool
  unlimit_lock_pages();
   ...
   *
   * Locks all pages mapped into the address space of the calling process (code,
   * data and stack segment, shared libraries, user space kernel data, shared
   * memory, and memory-mapped files) so that no page faults occur after calling
   * this function.
   *
   ...
   * effect is that realtime threads are not always started in pthread_create()
   * order. This phenomen is evident in regular kernels as well as in
   * PREEMPT_RT-patched kernels.
   *
